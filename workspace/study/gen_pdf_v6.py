...
C_BASELINE  = colors.HexColor('#1976D2')  # 基础题
C_TRAP      = colors.HexColor('#D32F2F')  # 易错题
C_ADVANCED  = colors.HexColor('#F57C00')  # 提分题
C_REAL      = colors.HexColor('#7B1FA2')  # 真题
C_REAL      = colors.HexColor('#C62828')  # 测试卷
C_REAL      = colors.HexColor('#C62828')  # 测试卷
C_EXAMPLE   = colors.HexColor('#2E7D32')  # 例题
C_VARIANT   = colors.HexColor('#E65100')  # 变式题
C_QTYPE_SC  = colors.HexColor('#1976D2')  # 选择题
C_QTYPE_FC  = colors.HexColor('#388E3C')  # 填空题
C_QTYPE_SA  = colors.HexColor('#F57C00')  # 解答题
C_QTYPE_MC  = colors.HexColor('#C62828')  # 多选题
...
def esc(text):
    """转义HTML特殊字符，清理所有不可见字符和多余空格"""
    t = str(text)
    # 删除所有不可打印字符(保留换行和普通空格)
    cleaned = []
    for c in t:
        cp = ord(c)
        if c == '\n' or c == ' ':
            ...
            continue
        elif cp == 0xa0:    # no-break space
            continue
        elif cp == 0x3000:  # ideographic space
            continue
        elif cp == 0x2003:  # em space
            continue
        elif cp == 0x2009:  # thin space
            continue
        elif cp == 0x200a:  # hair space
            continue
        elif cp == 0x200b:  # zero width space
            continue
        elif cp == 0x00ad:  # soft hyphen
            continue
        elif cp == 0xf0dc:  # private use
            continue
...
def split_opts(text):
    """拆分选项，处理 A．2B．3C．4D．5 这种连续格式"""
    text = text.replace('\xa0', '').replace('\x00', '')
    parts = re.split(r'(?=[A-D][．\.])', text)
    result = [p.strip() for p in parts if p.strip()]
    # 保留空选项但标记（如 "A．" 显示为 "A．____"）
    final = []
    for p in result:
        if re.match(r'^[A-D]．\s*$', p):
            final.append(p.replace('．', '．____'))  # 空选项用下划线占位
        else:
            final.append(p)
    return final
...
def classify_level(text):
    """分类题目难度层次"""
    text = text.lower()
    if '真题' in text or '2019' in text or '2020' in text or '2021' in text or '2022' in text or '2023' in text or '2024' in text:
        return 'real'
    if '易错' in text or '陷阱' in text or '常见错误' in text or '典型错误' in text:
        return 'trap'
    if '提分' in text or '拔尖' in text or '综合' in text:
        return 'advanced'
    return 'baseline'
...
def classify(text, in_topic=False):
    ...
    if OPT_PAT.match(text) and len(text) < 200: return 'option'
...
class SectionBanner(Flowable):
    """章节横幅：背景色块+白色标题"""
    def __init__(self, text, color, width=14*cm, height=26, font_size=11):
        Flowable.__init__(self)
...
class QTypeTag(Flowable):
    """题型标签：颜色区分"""
    def __init__(self, text, color, width=14*cm, height=18):
        Flowable.__init__(self)
        self._text = text; self._color = color; self._width = width; self.height = height; self.width = width
    def draw(self):
        c = self.canv
        c.setFillColor(self._color)
...
class LevelTag(Flowable):
    """分层训练标识"""
    def __init__(self, text, color, width=14*cm, height=16):
        Flowable.__init__(self)
        self._text = text; self._color = color; self._width = width; self.height = height; self.width = width
    def draw(self):
        c = self.canv
        c.setFillColor(self._color)
        c.roundRect(0, 0, 2.5, self.height, 1, fill=1, stroke=0)
        c.setFillColor(colors.white)
        c.setFont('Sans-Bold', 8)
        c.drawString(8, 3, self._text)
    ...
    for p in paras:
        ...
        if in_topic:
            ...
            if pt == 'qtype_tag':
                ...
                cur_q=None; continue
            if pt == 'example':
                cur_q = {'role':'example','text':text,'options':[],'sub_questions':[],'solution':{},'level':'advanced'}
                if cur_qt: cur_qt['questions'].append(cur_q); continue
            if pt == 'variant':
                cur_q = {'role':'variant','text':text,'options':[],'sub_questions':[],'solution':{},'level':'advanced'}
                if cur_qt: cur_qt['questions'].append(cur_q); continue
            ...
            if pt == 'question' and cur_qt:
                cur_q = {'role':'practice','text':text,'options':[],'sub_questions':[],'solution':{},'level':classify_level(text)}
...
def make_styles():
    ...
    ss.add(ParagraphStyle('CoverSlogan', fontName='Sans', fontSize=9,
    ...
    ss.add(ParagraphStyle('TOCItem', fontName='Sans', fontSize=9,
                          textColor=C_DARK, spaceBefore=1, spaceAfter=1, leading=13, leftIndent=12))
    # 内容
    ss.add(ParagraphStyle('Q', fontName='Sans', fontSize=10.5,
                          textColor=C_DARK, spaceBefore=3, spaceAfter=2, leading=17, leftIndent=0))
    ss.add(ParagraphStyle('Opt', fontName='Sans', fontSize=9,
                          textColor=C_DARK, spaceBefore=0.5, spaceAfter=0.5, leading=13, leftIndent=0))
    ss.add(ParagraphStyle('SubQ', fontName='Sans', fontSize=9.5,
                          textColor=C_DARK, spaceBefore=0.5, spaceAfter=0.5, leading=14, leftIndent=6))
    ss.add(ParagraphStyle('KPTxt', fontName='Sans', fontSize=9,
    ...
    ss.add(ParagraphStyle('AnsBody', fontName='Sans', fontSize=8.5,
                          textColor=colors.HexColor('#444'), spaceBefore=1, spaceAfter=1, leading=12, leftIndent=8))
    return ss

# ---- 题目渲染(题目册) ----
def get_level_color(level):
    if level == 'trap': return C_TRAP
    if level == 'advanced': return C_ADVANCED
    if level == 'real': return C_REAL
    return C_BASELINE

def get_qtype_color(qtype_name):
    if '选择' in qtype_name: return C_QTYPE_SC
    if '填空' in qtype_name: return C_QTYPE_FC
    if '解答' in qtype_name: return C_QTYPE_SA
    if '多选' in qtype_name: return C_QTYPE_MC
    return C_PRIMARY

def make_q_block(q, styles, q_num):
    elems = []; text = q.get('text',''); role = q.get('role','practice'); level = q.get('level','baseline')
    ...
    if role in ('example','variant'):
        ...
        elems.append(Paragraph(f'<font color="{hex_c(level_color)}"><b>{num}.</b></font>{multi} {esc(ct)}', styles['Q']))
    # 分层训练标识
    level_tag = ''
    if level == 'trap':
        level_tag = f'<font color="{hex_c(C_TRAP)}" size="7"> 易错</font>'
    elif level == 'advanced':
        level_tag = f'<font color="{hex_c(C_ADVANCED)}" size="7"> 提分</font>'
    elif level == 'real':
        level_tag = f'<font color="{hex_c(C_REAL)}" size="7"> 真题</font>'
    if level_tag and role not in ('example','variant'):
        elems[-1] = Paragraph(f'<font color="{hex_c(level_color)}"><b>{num}.</b></font>{multi} {esc(ct)}{level_tag}', styles['Q'])
    opts = q.get('options',[])
    if len(opts) >= 2:
        if len(opts) == 4:
...
def make_ans_block(q, styles, q_num, is_topic=False):
    ...
    if role == 'example':
        ...
        m = Q_NUM_PAT.match(text); num = m.group(1) if m else str(q_num)
        tag = f'<font color="{hex_c(level_color)}"><b>{num}.</b></font>'
...
def generate_pdf(sections, out_path, doc_type='题目册', is_ans=False):
    ...
    for sec in sections:
        if sec['kind'] == 'practice':
            ...
            story.append(SectionBanner(sec['title'][:32], C_PRIMARY, pw, 26, 11))
            ...
            for qt in sec.get('question_types',[]):
                qtype_color = get_qtype_color(qt['name'])
                story.append(QTypeTag(f'{qt["kind"]}：{qt["name"]}', qtype_color, pw)); story.append(Spacer(1, 3))
                for q in qt.get('questions',[]):
                    q_num += 1
            ...
            story.append(SectionMarker(sec['title'][:25])); story.append(PageBreak())
            story.append(SectionBanner(sec['title'][:38], C_REAL, pw, 28, 12))
            story.append(Spacer(1, 6))
            for q in sec.get('questions',[]):
                q_num += 1
                if is_ans: block = make_ans_block(q, styles, q_num)
...
def main():
    ...
    generate_pdf(q_secs, os.path.join(BASE, '高中数学同步练习_题目册_v6.pdf'), '题目册', False)
    generate_pdf(a_secs, os.path.join(BASE, '高中数学同步练习_答案册_v6.pdf'), '答案册', True)
    print("\n🎉 全部完成!")
