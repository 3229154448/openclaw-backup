#!/usr/bin/env python3
"""
高中数学同步练习册 · 印刷级PDF排版 v4
商业练习册水准：53天天练/一遍过/必刷题风格
- 思源黑体(标题) + 思源宋体(正文)
- 封面多层装饰 + 自动目录
- 知识点/题型/例题/变式 层级视觉区分
- 选项背景框 + 规范答题区
...
C_EXAMPLE   = colors.HexColor('#1565C0')
C_VARIANT   = colors.HexColor('#2E7D32')
C_QTYPE     = colors.HexColor('#E65100')
...
def classify(text, in_topic=False):
    ...
    # 数字+括号缺分隔：如 "3（2023·四川）" "16（2023·湖南）"
...
class SectionMarker(Flowable):
    ...
    def draw(self):
        # 将章节名存到canvas的_doc namespace中
        self.canv._doc.chapter_name = self._name

class AnsSpace(Flowable):
    """规范答题区：带"解："提示的虚线格"""
    def __init__(self, lines=5, w=14*cm):
        Flowable.__init__(self)
        self.sw = w; self.lc = lines
        self.height = lines * 18 + 14; self.width = w
    def draw(self):
        c = self.canv
        c.setFont('Serif', 9); c.setFillColor(C_GRAY)
        c.drawString(0, self.height - 11, '解：')
        c.setStrokeColor(colors.HexColor('#D0D0D0'))
        c.setLineWidth(0.3); c.setDash(3, 3)
        for i in range(self.lc):
            y = self.height - 14 - (i + 1) * 18
...
def make_styles():
    ...
    # 内容
    ss.add(ParagraphStyle('Q', fontName='Serif', fontSize=10,
                          textColor=C_DARK, spaceBefore=3, spaceAfter=1, leading=15))
    ss.add(ParagraphStyle('Opt', fontName='Serif', fontSize=9.5,
                          textColor=C_DARK, spaceBefore=1, spaceAfter=1, leading=13.5, leftIndent=18))
...
def make_q_block(q, styles, q_num):
    ...
    if role not in ('example','variant'):
        if '解答' in q.get('text','') or len(q.get('sub_questions',[])) >= 2:
            elems.append(Spacer(1,4)); elems.append(AnsSpace(5, 14*cm))
    return elems
...
def main():
    ...
    generate_pdf(q_secs, os.path.join(BASE, '高中数学同步练习_题目册_v4.pdf'), '题目册', False)
    generate_pdf(a_secs, os.path.join(BASE, '高中数学同步练习_答案册_v4.pdf'), '答案册', True)
    print("\n🎉 全部完成!")
